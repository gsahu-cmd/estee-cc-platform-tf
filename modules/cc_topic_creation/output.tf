output "topic_ids" {
	...
	value       = { for topic_name, topic in confluent_kafka_topic.topic : topic_name => topic.id }
}

output "topic_names" {
	description = "Kafka topic names created by this module."
	value       = { for topic_name, topic in confluent_kafka_topic.topic : topic_name => topic.topic_name }
}
