# Environment Variables
elc_env_name           = "elc-prod-eastus2"
elc_env_stream_package = "ESSENTIALS"

# Gateway variables
elc_ingress_gateway_name = "elc-prod-eastus2-ingress-gateway"
elc_cluster_region       = "eastus2"

# Access point
elc_private_endpoints = {
  "elc-prod-eastus2-pe-01" = {
    elc_mod_display_name                 = "elc-prod-eastus2-ap-01"
    elc_mod_private_endpoint_resource_id = "/subscriptions/<subscription-id>/resourceGroups/<resource-group>/providers/Microsoft.Network/privateEndpoints/pe-gyan-test-eastus2"
  }
}

# Cluster Variables
elc_cluster_name         = "elc-prod-eastus2-core-enterprise"
elc_cluster_cloud        = "AZURE"
elc_cluster_region       = "eastus2"
elc_cluster_availability = "SINGLE_ZONE"
